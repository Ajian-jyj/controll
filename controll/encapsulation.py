...
def set_Board_Mode(num):
    ...
    且第三个参数没有
    '''
    out_mode = Out_Mode.Double_Pluse.value
    ...
    '''
    set_alm_logic用于设置一个轴报警信号是高电平有效还是低电平有效。
    1 表示外部报警开关高电平触发控制器；
    0 表示外部报警开关低电平触发控制器。
    '''
    flag = Logic_Flag.LOW.value
    statue = myController.set_alm_logic(num, flag)
    if statue == -1:
        raise Exception("错误:set_alm_logic外部报警信号有限点电平使能失败")

    flag = Logic_Flag.HIGH.value
    statue = myController.enable_alm(num, flag)
    if statue == -1:
    ...
    '''
    set_el_logic用于设置一个轴限位信号是高电平有效还是低电平有效。
    1 表示外部限位开关高电平触发控制器；
    0 表示外部限位开关低电平触发控制器。
    '''
    flag = Logic_Flag.HIGH.value
    ...
    if statue == -1:
        raise Exception("错误:set_el_logic外部限位信号有效电平设置失败")

    # 外部限位信号使能
    flag = Logic_Flag.HIGH.value
    statue = myController.enable_el(num, flag)
    if statue == -1:
        raise Exception("错误:enable_el外部限位信号使能失败")
    else:
        check_limit(num)
    ...
    '''
    flag = Logic_Flag.LOW.value
    statue = myController.set_card_alm_logic(cardno, flag)
    if statue == -1:
        raise Exception("错误:set_card_alm_logic板卡报警信号有限点电平使能失败")

    flag = Logic_Flag.HIGH.value
    statue = myController.enable_card_alm(num, flag)
    if statue == -1:
...
def check_card_alarm():
    '''
    检查板卡的报警信号是否有效
    '''
    statue = myController.check_card_alarm(cardno)
    if statue == 0:
        raise Exception("错误:板卡报警信号无效")
    elif statue == -3:
        raise Exception("错误:check_card_alarm运行异常")
    else:
        print('check_card_alarm报警信号有效')


def check_sfr(cardno):
    '''
    读取专用输入口所有的开关量状态
    状态位    专用信号    说明
    ...
    '''
    statue = myController.check_sfr(cardno)
    if statue == -1:
...
def check_sfr_bit(bitno):
    '''
    读取专用输入口某位的开关量状态
    bitno：表示第几个输入口，取值范围为 1~21。
    '''
    statue = myController.check_sfr_bit(cardno, bitno)
    if statue == -1:
...
def power_on():
    ...
    电机上电,给予电位0为上电
    '''
    res_1 = myController.outport_bit(cardno, Axis_Num.Axis_NO_1.value, 0)
    res_2 = myController.outport_bit(cardno, Axis_Num.Axis_NO_2.value, 0)
...
def power_off():
    ...
    电机下电,给予电位1为下电
    '''
    res_1 = myController.outport_bit(cardno, 1, 0)
    res_2 = myController.outport_bit(cardno, 2, 0)
...
def set_profile(num, low_speed=10000, high_speed=5000, acceleration=80000, deceleration=80000):
    '''
    设置快速运动的低速、高速和加速度
    '''
...
def con_pmove2(num_1, step_1, num_2, step_2):
    '''
    2 个轴以常速做相对位置点位运动
    '''
...
def fast_pmove2(num_1, step_1, num_2, step_2):
    '''
    2 个轴以快速做点位运动
    '''
    res = myController.con_pmove2(num_1, step_1, num_2, step_2)
...
def con_vmove2(num_1, dir1, num_2, dir2):
    '''
    两个轴以常速做连续运动
    '''
...
def fast_vmove2(num_1, dir1, num_2, dir2):
    '''
    两个轴以快速做连续运动
    '''
...
def con_hmove2(num1, dir1, num2, dir2):
    '''
    两个轴以常速做回原点运动
    '''
...
def fast_hmove2(num1, dir1, num2, dir2):
    '''
    两个轴以快速做回原点运动
    '''
...
def con_line2(num1, pos1, num2, pos2):
    '''
    两个轴做常速直线运动
    '''
...
def fast_line2(num1, pos1, num2, pos2):
    '''
    两个轴做快速直线运动
    '''
...
def set_s_section(num, accel_sec=100, decel_sec=200):
    '''
    设置轴的 S 形升降速的 S 段。
